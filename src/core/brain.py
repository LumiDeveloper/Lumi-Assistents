...
class LumiBrain:
    def __init__(self):
        ...
        self.system_prompt = (
            ...
            "2. В САМОМ КОНЦЕ ответа добавить технический тег: [EXEC: название]"

            "Примеры:"
            "- 'Сделаю! [EXEC: notepad]'"
            "- 'Запускаю твой редактор кода. [EXEC: code]'"
            "- 'Сейчас посчитаем... [EXEC: calc], но пожалуйства ищи именно calc, а не ms-calculator, у меня возможно его нет'"
            "- 'Если простит запустить браузер [EXEC: firefox], так как Хрома на этой системе нету.'"

            "Важно: Используй только формат [EXEC: ...]. Если не уверена в названии программы, используй самое простое (например, 'firefox', 'spotify', 'vlc')."
        )
